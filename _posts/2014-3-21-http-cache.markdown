...
>* no-cache：访问此页面不会在Internet临时文件夹留下页面备份，每次在释放缓存副本之前都强制发送请求给源服务器进行验证，这在确保认证有效性上很管用（和public结合使用）或者保证内容必须是即时的，不得无视缓存的所有优点，如国内的微博、twitter等的刷新显示Add。
>* no-store：强制缓存在任何情况下都不要保留任何副本。如果确实不需要缓存，可以 no-cache 和 no-store 配合使用        
